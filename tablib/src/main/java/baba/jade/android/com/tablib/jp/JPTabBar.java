...
import baba.jade.android.com.tablib.jp.animate.AnimationType;
import baba.jade.android.com.tablib.jp.animate.BouncingAnimater;
import baba.jade.android.com.tablib.jp.animate.FlipAnimater;
import baba.jade.android.com.tablib.jp.animate.JumpAnimater;
import baba.jade.android.com.tablib.jp.animate.RotateAnimater;
import baba.jade.android.com.tablib.jp.animate.Scale2Animater;
import baba.jade.android.com.tablib.jp.animate.ScaleAnimater;
import baba.jade.android.com.tablib.jp.anno.NorIcons;
import baba.jade.android.com.tablib.jp.anno.SeleIcons;
import baba.jade.android.com.tablib.jp.anno.Titles;
...
public class JPTabBar extends LinearLayout implements ViewPager.OnPageChangeListener {
  ...
  private void initFromAttribute() {
    int normalColor =
        mAttribute.getColor(R.styleable.JPTabBar_TabNormalColor, DEFAULT_NORMAL_COLOR);
    int selectColor =
        mAttribute.getColor(R.styleable.JPTabBar_TabSelectColor, DEFAULT_SELECT_COLOR);
    int textSize = DensityUtils.px2sp(mContext,
        mAttribute.getDimensionPixelSize(R.styleable.JPTabBar_TabTextSize,
            DensityUtils.sp2px(mContext, DEFAULT_TEXTSIZE)));
    //这里
    int iconSize = mAttribute.getDimensionPixelSize(R.styleable.JPTabBar_TabIconSize,
        DensityUtils.dp2px(mContext, DEFAULT_ICONSIZE));
    int margin = mAttribute.getDimensionPixelOffset(R.styleable.JPTabBar_TabMargin,
        DensityUtils.dp2px(mContext, DEFAULT_MARGIN));
    AnimationType AnimateType =
        AnimationType.values()[mAttribute.getInt(R.styleable.JPTabBar_TabAnimate,
            AnimationType.NONE.ordinal())];
    int BadgeColor = mAttribute.getColor(R.styleable.JPTabBar_BadgeColor, DEFAULT_BADGE_COLOR);
    int BadgetextSize = DensityUtils.px2sp(mContext,
        mAttribute.getDimensionPixelSize(R.styleable.JPTabBar_BadgeTextSize,
            DensityUtils.sp2px(mContext, DEFAULT_BADGE_TEXTSIZE)));
    int badgePadding = DensityUtils.px2dp(mContext,
        mAttribute.getDimensionPixelOffset(R.styleable.JPTabBar_BadgePadding,
            DensityUtils.dp2px(mContext, DEFAULT_PADDING)));
    int badgeVerMargin = DensityUtils.px2dp(mContext,
        mAttribute.getDimensionPixelOffset(R.styleable.JPTabBar_BadgeVerticalMargin,
            DensityUtils.dp2px(mContext, DEFAULT_BADGEVERTICAL_MARGIN)));
    int badgeHorMargin = DensityUtils.px2dp(mContext,
        mAttribute.getDimensionPixelOffset(R.styleable.JPTabBar_BadgeHorizonalMargin,
            DensityUtils.dp2px(mContext, DEFAULT_BADGEHORIZONAL_MARGIN)));
    int hMargin = mAttribute.getDimensionPixelOffset(R.styleable.JPTabBar_TabMiddleHMargin,
        DensityUtils.dp2px(mContext, DEFAULT_MIDDLEMARGIN));
    boolean acceptFilter =
        mAttribute.getBoolean(R.styleable.JPTabBar_TabIconFilter, DEFAULT_ACEEPTFILTER);
    ...
    if (!isInEditMode()) {
      ...
      for (int i = 0; i < mJPTabItems.length; i++) {
        final int temp = i;
        Animatable animater = AnimateType == AnimationType.SCALE ? new ScaleAnimater()
            : AnimateType == AnimationType.ROTATE ? new RotateAnimater()
                : AnimateType == AnimationType.FLIP ? new FlipAnimater()
                    : AnimateType == AnimationType.JUMP ? new JumpAnimater()
                        : AnimateType == AnimationType.SCALE2 ? new Scale2Animater() : null;
        mJPTabItems[i] =
            new JPTabItem.Builder(mContext).setTitle(mTitles == null ? null : mTitles[i])
                .setIndex(temp)
            ...
            switch (event.getAction()) {
              case MotionEvent.ACTION_DOWN:
                if (mJPTabItems[mSelectIndex].getAnimater() != null) {
                  mJPTabItems[mSelectIndex].getAnimater()
                      .onPressDown(mJPTabItems[mSelectIndex].getIconView(), true);
              ...
              case MotionEvent.ACTION_UP:
                if (isInRect(v, event)) {
                  //在触摸的item内
                  if (mTabPager != null
                      && mTabPager.getAdapter() != null
                      && mTabPager.getAdapter().getCount() >= mJPTabItems.length) {
                    mNeedAnimate = true;
                    mTabPager.setCurrentItem(temp, false);
                  } else if (mTabPager != null
                      && mTabPager.getAdapter() != null
                      && mTabPager.getAdapter().getCount() <= mJPTabItems.length) {
                    mNeedAnimate = true;
                    mTabPager.setCurrentItem(temp, false);
                  ...
                  if (mJPTabItems[mSelectIndex].getAnimater() != null) {
                    mJPTabItems[mSelectIndex].getAnimater()
                        .onTouchOut(mJPTabItems[mSelectIndex].getIconView(), true);
        ...
        if (i == (mJPTabItems.length / 2 - 1)
            && mAttribute.getResourceId(R.styleable.JPTabBar_TabMiddleView, 0) != 0) {

          //添加中间的占位距离控件
          View stement_view = new View(mContext);
          ViewGroup.LayoutParams params =
              new ViewGroup.LayoutParams(hMargin, ViewGroup.LayoutParams.MATCH_PARENT);
  ...
  private void CheckIfAssertError(String[] titles, int[] normalIcon, int[] selectedIcon) {
    ...
    //判断注解里面的数组长度是否一样
    if ((mTitles != null && originlen != titles.length) || (selectedIcon != null
        && originlen != selectedIcon.length)) {
      throw new TabException(
          "Every Array length is not equal,Please Check Your Annotation in your Activity,Ensure Every Array length is equals!");
    }
  }
  ...
  private boolean isInRect(View v, MotionEvent e) {
    ...
    RectF rectF = new RectF(location[0], location[1], location[0] + v.getWidth(),
  ...
  private void fillMiddleParams() {
    int bottom_dis = mAttribute.getDimensionPixelSize(R.styleable.JPTabBar_TabMiddleBottomDis,
        DensityUtils.dp2px(mContext, DEFAULT_MIDDLEICONBOTTOM));
    if (getParent().getClass().equals(RelativeLayout.class)) {
      RelativeLayout.LayoutParams params =
          (RelativeLayout.LayoutParams) mMiddleItem.getLayoutParams();
      params.setMargins(0, 0, 0, bottom_dis);
      //params.addRule(RelativeLayout.ALIGN_PARENT_BOTTOM);//源代码 默认在底部
  ...
  public void setContainer(ViewPager pager) {
    if (pager != null) {
      mTabPager = pager;
      mTabPager.addOnPageChangeListener(this);
    }
  }
  ...
  public void hideBadge(int position) {
    if (mJPTabItems != null) mJPTabItems[position].hiddenBadge();
  }
  ...
  public void generate() {
    if (mJPTabItems == null) initFromAttribute();
  }
  ...
  public void setAnimation(AnimationType animationType) {
    for (int i = 0; i < mJPTabItems.length; i++) {
      mJPTabItems[i].setAnimater(animationType == AnimationType.SCALE ? new ScaleAnimater()
          : animationType == AnimationType.ROTATE ? new RotateAnimater()
              : animationType == AnimationType.JUMP ? new JumpAnimater()
                  : animationType == AnimationType.FLIP ? new FlipAnimater()
                      : animationType == AnimationType.SCALE2 ? new Scale2Animater() : null);
  ...
  public boolean isBadgeShow(int index) {
    if (mJPTabItems != null) return mJPTabItems[index].isBadgeShow();
  ...
  public void setTabMargin(int margin) {
    if (mJPTabItems != null) {
      for (JPTabItem item : mJPTabItems) {
        ((RelativeLayout.LayoutParams) item.getIconView().getLayoutParams()).topMargin =
            DensityUtils.dp2px(mContext, margin);
      }
    }
  }
  ...
  @Override public void onPageScrolled(int position, float positionOffset,
    ...
    if (mJPTabItems == null
        || position > mJPTabItems.length - 1
        || 1 + position > mJPTabItems.length - 1) {
      return;
    }
    if (positionOffset > 0f) {
      ...
      if (mJPTabItems[position].getAnimater() != null && mNeedScrollAnimate) {
        if (mJPTabItems[position].getAnimater().isNeedPageAnimate()) {
          mNeedAnimate = false;
          mJPTabItems[position].getAnimater()
              .onPageAnimate(mJPTabItems[position].getIconView(), 1 - positionOffset);
          mJPTabItems[position + 1].getAnimater()
              .onPageAnimate(mJPTabItems[position + 1].getIconView(), positionOffset);
        } else {
          mNeedAnimate = true;
        }
      } else {
