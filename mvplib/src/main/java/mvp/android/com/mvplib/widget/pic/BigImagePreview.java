package mvp.android.com.mvplib.widget.pic;
...
public class BigImagePreview extends RelativeLayout {
  ...
  public void init(final List<?> list, List<Integer> widthList, List<Integer> heightList,
    ...
    galleryView.setItemClickListener(new GalleryView.ItemClickListener() {
      @Override public void click(int x, int y, float width, float height, final int position) {
  ...
  public void setGalleryLoadMoreData(final List<?> list, List<Integer> widthList,
  ...
  private void galleryItemClick(final int x, final int y, final float width, final float height,
      final int position) {
    ...
    Glide.with(mContext)
        .load(urlList.get(position))
        .override((int) imgWidth, (int) imgHeight)
        .dontAnimate()
        .into(transBigImageView);
  ...
  class ViewPagerAdapter extends PagerAdapter {
    ...
    @Override public Object instantiateItem(ViewGroup container, final int i) {
      ImageView imageView = new ImageView(mContext);
  ...
  public void TransSmallImageViewToExit(int position) {
    ...
    mTransSmallImageView.exit((int) (info.get(0) / 1), (int) (info.get(1) / 1), info.get(2),
        info.get(3));
  }

  public void setDefShow(final int position, List<String> list) {
    final List<Float> info = galleryView.getChildInfo(position);
    gridPosition = position;
    new Handler().postDelayed(new Runnable() {
      @Override public void run() {
        galleryView.getChildAt(position).setVisibility(INVISIBLE);
    ...
    galleryView.getChildAt(urlList.size() + list.size() - 1).post(new Runnable() {
      @Override public void run() {
        galleryItemClick((int) (info.get(0) / 1), (int) (info.get(1) / 1), info.get(2), info.get(3),
            position);
      }
    });
  }
